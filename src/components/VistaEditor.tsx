...
export default function VistaEditor({
  ...
  const createNewScene = () => {
    const sceneName = prompt('Enter scene name:', 'New Vista Scene') || 'New Vista Scene';
    const newScene = createEmptyScene(sceneName);
    setCurrentScene(newScene);
  ...
  const startEditingSceneName = () => {
    if (currentScene && !readOnly) {
  ...
  const saveSceneName = () => {
    if (currentScene && editingSceneName.trim()) {
      const updatedScene = { ...currentScene, name: editingSceneName.trim() };
      setCurrentScene(updatedScene);
      addToHistory(updatedScene);
      if (onSceneChange) onSceneChange(updatedScene);
    }
    setIsEditingSceneName(false);
    setEditingSceneName('');
  };

  const cancelEditingSceneName = () => {
    setIsEditingSceneName(false);
    setEditingSceneName('');
  };
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'Escape') cancelEditingSceneName();
                }}
              ...
              <button
                onClick={cancelEditingSceneName}
